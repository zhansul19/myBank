...
import (
	...
	mockdb "github.com/zhansul19/myBank/db/mock"
	...
	testCases := []struct {
	...
	}{
		{
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				...
				bodyDataTest(t, recorder.Body, account)
			},
		},
		{
			name:      "NotFound",
		...
		},
		{
			...
			accountID: account.ID,
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, "random", time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					GetAccount(gomock.Any(), gomock.Eq(account.ID)).
...
func bodyDataTest(t *testing.T, body *bytes.Buffer, account db.Accounts) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			checkResponse: func(recoder *httptest.ResponseRecorder) {
				...
				bodyDataTest(t, recoder.Body, account)
			},
		},
		{
			name: "InternalError",
		...
		// 		}
		// 		store.EXPECT().CreateAccount(gomock.Any(), gomock.Eq(arg)).
		// 			Times(1).Return(db.Accounts{}, errors.New("foreign_key_violation"))
		// 	},
		// 	checkResponse: func(recoder *httptest.ResponseRecorder) {
